...
preview`, and `docker compose up` on port 8080. The container serves assets
  byte-identical to a local build of the same commit, so the Docker check is
  about packaging rather than a second pass over the app.
